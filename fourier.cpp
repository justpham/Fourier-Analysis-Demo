...
// files to read and write are now provided via command-line args
...
{
	MPI_Status status;
	const char * signalFile = NULL;
	const char * plotFile = NULL;
	...
	if( argc < 3 )
	{
		if( me == THEBOSS )
		{
			fprintf( stderr, "Usage: %s <signal_input_file> <plot_output_file>\n", argv[0] );
		}
		MPI_Finalize( );
		return -1;
	}
	...
	if( me == THEBOSS )
	{
		...
		{
			if( dst != THEBOSS )
				MPI_Send( &BigSignal[dst*PPSize], PPSize, MPI_FLOAT, dst, 'G', MPI_COMM_WORLD );
		}
	}
	else
	{
		// have everyone else receive from the THEBOSS:
		MPI_Recv( PPSignal, PPSize, MPI_FLOAT, THEBOSS, 'G', MPI_COMM_WORLD, &status );
	}
	...
	else
	{
		...
		MPI_Send( PPSums, MAXPERIODS, MPI_FLOAT, THEBOSS, 'S', MPI_COMM_WORLD );
	}
	...
	if( me == THEBOSS )
	{
		...
		for( int src = 0; src < NumCpus; src++ )
		{
			if( src != THEBOSS )
			{
				MPI_Recv( tmpSums, MAXPERIODS, MPI_FLOAT, src, 'S', MPI_COMM_WORLD, &status );
